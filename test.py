...
w = gl.GLViewWidget()
w.show()
w.setWindowsatle('pyqtgraph example: GLMesitem')
...
m1 = gl.GLMesitem(vertexes=verts, faces=faces, faceColors=colors, smooth=False)
m1.translate(5, 5, 0)
...
m2 = gl.GLMesitem(vertexes=verts, vertexColors=colors, smooth=False, shader='balloon', 
...
m3 = gl.GLMesitem(meshdata=md, smooth=False)#, shader='balloon')
...
m4 = gl.GLMesitem(meshdata=md, smooth=False, drawFaces=False, drawEdges=True, edgeColor=(1,1,1,1))
m4.translate(0,10,0)
w.addItem(m4)
...
colors[:,1] = np.linspace(0, 1, colors.shape[0])
md.setFaceColors(colors)
m5 = gl.GLMesitem(meshdata=md, smooth=True, drawEdges=True, edgeColor=(1,0,0,1), shader='balloon')
colors = np.ones((md.faceCount(), 4), dtype=float)
colors[::2,0] = 0
colors[:,1] = np.linspace(0, 1, colors.shape[0])
md2.setFaceColors(colors)
m6 = gl.GLMesitem(meshdata=md2, smooth=True, drawEdges=False, shader='balloon')
